  ...
  if($db->getRowsNum($result)) {
    ...
    while($member = $db->fetchArray($result)) {
      ...
      echo '<tr bgcolor="#eeeeee">
        ...
        <td>', $member['title'], '</td>
<td><a href="?op=members_edit&action=edit&id=', $member['id'], '">', '<img src='. $pathIcon16 .'/edit.png' . ' alt='  . _EDIT .  ' title='  . _EDIT .  ' />', '</a>  <a href="?op=members_delete&action=confirm&id=', $member['id'], '">', '<img src='. $pathIcon16 .'/delete.png' . ' alt='  . _DELETE .  ' title='  . _DELETE .  ' />','</a>
         <a href="?op=members_approve&action=approve&id=', $member['id'], '&subaction=', $member['member'] ? '0' : '1', '">', !$member['member'] ? '<img src='. $pathIcon16 .'/on.png' . ' alt='  . _MD_APPROVE .  ' title='  . _MD_APPROVE .  ' />' : '<img src='. $pathIcon16 .'/off.png' . ' alt='  . _MD_DISAPPROVE .  ' title='  . _MD_DISAPPROVE .  ' />', '</a>',
        '</td></tr>';
    }
    echo '</table>';
  } else
      ...
      if($db->getRowsNum($result)) {
        ...
        while($row = $db->fetchArray($result)) {
          echo '<tr>
            ...
            <td><a href="?op=Group_Editor&g_action=edit&id=', $row['id'], '">', '<img src='. $pathIcon16 .'/edit.png' . ' alt='  . _EDIT .  ' title='  . _EDIT .  ' />', '</a>
            <a href="?op=Group_Editor&g_action=delete&id=', $row['id'], '">', '<img src='. $pathIcon16 .'/delete.png' . ' alt='  . _DELETE .  ' title='  . _DELETE .  ' />', '</a></td></tr>';
        }
        echo '</table>';
      } else
    ...
    if($db->getRowsNum($result) > 0) {
      ...
      while($row = $db->fetchArray($result)) {
        echo '<tr bgcolor="#eeeeee">
          ...
          <td><a href="?op=Category_Editor&c_action=edit&id=', $row['id'], '&name=', $row['name'], '&weight=', $row['weight'], '&active=', $row['active'], '">', '<img src='. $pathIcon16 .'/edit.png' . ' alt='  . _EDIT .  ' title='  . _EDIT .  ' />', '</a>  <a href="?op=Category_Editor&c_action=delete&id=', $row['id'], '">', '<img src='. $pathIcon16 .'/delete.png' . ' alt='  . _DELETE .  ' title='  . _DELETE .  ' />', '</a></td></tr>';
      }
      echo '</table>';
    } else
...
echo 'ADDDDDDDD';
            if(!isset($form_params))
              $form_params = array('id'=>0, 't_action'=>'insert', 'btn_label'=>_MD_INSERT);
            echo '<form method="post">';
            if($form_params['id'])
              echo '<input type="hidden" name="id" value="', $form_params['id'], '">';
//            echo("<input type=\"hidden\" name=\"t_action\" value=\"insert\">");
      ...
      break;
        ...
        $titles_assoc[$row['id']] = $row['name'];
			...
			if(count($titles) > 0){
				echo("<p><table cellspacing=0 cellpadding=8 class=\"outer\">");
				echo("<tr bgcolor=\"#cccccc\"><td>"._MD_TITLENAME."</td><td>"._MD_WEIGHT."</td><td>"._MD_ACTIONS."</td></tr>");
					for($j=0;$j<count($titles);$j++){
						echo("<tr bgcolor=\"#eeeeee\">");
						echo("<td>".$titles[$j]['name']."</td>");
						echo("<td>".$titles[$j]['weight']."</td>");
                        echo("<td><a href=\"?op=Title_Editor&t_action=edit&id=".$titles[$j]['id']."&name=".$titles[$j]['name']."&weight=".$titles[$j]['weight']."\">".'<img src='. $pathIcon16 .'/edit.png' . ' alt='  . _EDIT .  ' title='  . _EDIT .  ' />'."</a>");
						echo("<a href=\"?op=Title_Editor&t_action=delete&id=".$titles[$j]['id']."\">".'<img src='. $pathIcon16 .'/delete.png' . ' alt='  . _DELETE .  ' title='  . _DELETE .  ' />'."</a></td>");
						echo("</tr>");
					}
				echo("</table>");
        // Default title
